...
const markdownComponents = {
  h1: ({ node, ...props }) => <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-3 mb-1.5 border-b border-slate-200 dark:border-slate-800 pb-1" {...props} />,
  h2: ({ node, ...props }) => <h2 className="text-base font-bold text-emerald-800 dark:text-emerald-300 mt-3 mb-1.5" {...props} />,
  h3: ({ node, ...props }) => <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 mt-2 mb-1" {...props} />,
  p: ({ node, ...props }) => <p className="mb-2 leading-relaxed text-slate-700 dark:text-slate-300 font-normal last:mb-0" {...props} />,
  ...
  blockquote: ({ node, ...props }) => (
    <blockquote className="border-l-4 border-emerald-500 pl-3 py-1 my-2 bg-emerald-50/50 dark:bg-emerald-950/40 rounded-r-lg text-slate-700 dark:text-slate-300 italic text-xs" {...props} />
  ),
  table: ({ node, ...props }) => (
    <div className="overflow-x-auto my-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
      <table className="w-full text-left text-xs border-collapse bg-white dark:bg-slate-900" {...props} />
    </div>
  ),
  thead: ({ node, ...props }) => <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold border-b border-slate-200 dark:border-slate-700" {...props} />,
  tr: ({ node, ...props }) => <tr className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50/60 dark:hover:bg-slate-800/40" {...props} />,
  th: ({ node, ...props }) => <th className="p-2.5 font-semibold text-slate-800 dark:text-slate-200 border-r border-slate-200 dark:border-slate-700 last:border-r-0" {...props} />,
  td: ({ node, ...props }) => <td className="p-2.5 text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800 last:border-r-0" {...props} />,
  a: ({ node, ...props }) => (
    <a className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-medium underline transition" target="_blank" rel="noopener noreferrer" {...props} />
  ),
  hr: ({ node, ...props }) => <hr className="my-3 border-slate-200 dark:border-slate-800" {...props} />,
  code: ({ node, inline, className, children, ...props }) => {
    if (inline) {
      return (
        <code className="bg-slate-100 dark:bg-slate-800 text-emerald-800 dark:text-emerald-300 font-mono text-[12px] px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700" {...props}>
          {children}
        </code>
      );
    }
    ...
    return (
      <div
        ...
        className={`group relative flex items-center justify-between p-2.5 rounded-xl cursor-pointer text-sm transition duration-150 border ${
          isActive
            ? "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-900 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800 font-semibold"
            : "hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border-transparent hover:border-slate-200 dark:hover:border-slate-700"
        }`}
      >
        <div className="flex items-center gap-2 overflow-hidden w-full pr-24">
          ...
          {isEditing ? (
            <input
              ...
              className="text-xs bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 border border-emerald-400 dark:border-emerald-600 rounded px-2 py-1 w-full focus:outline-none"
            />
          ) : (
            <div className="flex flex-col overflow-hidden">
              <span className="truncate text-xs font-medium text-slate-800 dark:text-slate-200 group-hover:text-emerald-700 dark:group-hover:text-emerald-400">
        ...
        {!isEditing && (
          <div className="absolute right-1 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white/95 dark:bg-slate-800/95 p-1 rounded-lg shadow-xs">
            ...
            <button
              ...
              className="p-1 text-slate-400 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition text-xs"
            >
              ⬇
            </button>
            {/* Pin Button */}
            <button
              ...
              className={`p-1 transition text-xs ${isPinned ? "text-amber-600 dark:text-amber-400" : "text-slate-400 hover:text-amber-500"}`}
            >
              📌
            </button>
            {/* Favorite Button */}
            <button
              ...
              className={`p-1 transition text-xs ${isFavorite ? "text-yellow-500 dark:text-yellow-400" : "text-slate-400 hover:text-yellow-500"}`}
            >
              ⭐
            </button>
            {/* Rename Button */}
            <button
              ...
              className="p-1 text-slate-400 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition"
            >
            ...
            <button
              ...
              className="p-1 text-slate-400 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5">
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 p-4 transition-colors">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <button
          ...
          className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium py-2.5 px-3 rounded-xl border border-slate-200 dark:border-slate-700 transition duration-200 cursor-pointer text-xs"
        >
          <span>⬆</span> Import
        </button>
      </div>
      ...
      <div className="relative mb-3 flex-shrink-0">
        ...
        <input
          ...
          className="w-full pl-8 pr-8 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 transition text-slate-700 dark:text-slate-200 font-medium"
        />
        {searchQuery && (
          <button
          ...
          <>
            ...
            {pinnedChats.length > 0 && (
              <div className="space-y-1">
                <div className="text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider px-1 mb-1 flex items-center gap-1">
                  <span>📌</span> Pinned
                </div>
            ...
            {favoriteChats.length > 0 && (
              <div className="space-y-1">
                <div className="text-[10px] font-bold text-yellow-600 dark:text-yellow-400 uppercase tracking-wider px-1 mb-1 flex items-center gap-1">
                  <span>⭐</span> Favorites
                </div>
      ...
      <div className="mt-4 border-t border-slate-100 dark:border-slate-800 pt-3 flex-shrink-0">
        ...
        <div className="flex flex-wrap gap-1.5">
          {quickChips.slice(0, 3).map((chip, idx) => (
            <button
              ...
              className="text-[11px] px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/60 hover:text-emerald-700 dark:hover:text-emerald-300 border border-slate-200/60 dark:border-slate-700 rounded-lg transition font-medium text-slate-700 dark:text-slate-300 cursor-pointer"
            >
              {chip}
            </button>
          ))}
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row gap-6">
        ...
        <aside className="hidden md:block w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex-shrink-0 overflow-hidden h-[650px]">
        ...
        {mobileSidebarOpen && (
          <div className="fixed inset-0 z-50 flex md:hidden">
            ...
            ></div>
            <div className="relative w-80 bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col z-10">
              <div className="p-3 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
                <span className="text-sm font-bold text-slate-800 dark:text-slate-100">Chat History</span>
                <button
                  onClick={() => setMobileSidebarOpen(false)}
        ...
        <section className="flex-grow bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col h-[650px] overflow-hidden">
          {/* Chat Header */}
          <div className="bg-gradient-to-r from-emerald-600 to-green-700 dark:from-emerald-700 dark:to-green-800 p-4 text-white flex items-center justify-between">
          ...
          <div className="bg-slate-50/70 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-800 px-4 py-2 flex items-center gap-2 overflow-x-auto text-xs">
            <span className="text-slate-400 dark:text-slate-500 font-medium flex-shrink-0">Suggestions:</span>
            {quickChips.map((chip, idx) => (
              <button
                key={idx}
                onClick={() => handleChipClick(chip)}
                className="flex-shrink-0 text-[11px] px-3 py-1 bg-white dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/60 hover:text-emerald-700 dark:hover:text-emerald-300 border border-slate-200/70 dark:border-slate-700 rounded-full font-medium text-slate-600 dark:text-slate-300 transition cursor-pointer"
              >
                {chip}
              </button>
            ))}
          ...
          <div className="flex-grow p-6 overflow-y-auto space-y-6 bg-slate-50/30 dark:bg-slate-950/30">
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div className="relative group max-w-[85%] sm:max-w-[80%]">
                  <div
                    className={`rounded-2xl p-4 shadow-sm text-sm ${
                      msg.sender === "user"
                        ...
                        : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-100 dark:border-slate-700/70 rounded-tl-none pr-10"
                    }`}
                  >
                    {msg.sender === "user" ? (
                      msg.text
                  ...
                  {msg.sender === "bot" && (
                    <button
                      ...
                      className="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 rounded-lg transition-all opacity-80 hover:opacity-100 cursor-pointer flex items-center gap-1 text-[11px] font-medium"
                    >
                ...
                <span className="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 px-2">
                  {msg.time}
                </span>
              </div>
            ))}
          ...
          <form
            ...
            className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center gap-3"
          >
            <input
              type="text"
              value={inputText}
              ...
              className="flex-grow px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 text-sm transition-colors text-slate-700 dark:text-slate-100 font-medium"
            />
            <button
              type="submit"
              disabled={loading}
